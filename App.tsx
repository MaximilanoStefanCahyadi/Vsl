...
const App: React.FC = () => {
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case 'loading':
        return <LoadingScreen key="loading" onComplete={() => setCurrentPage('next_chapter')} />;
      case 'next_chapter':
        return (
            <div className="flex items-center justify-center h-screen bg-fred-pink text-center p-8 text-white">
                <div>
                    <h1 className="text-4xl font-serif font-bold mb-4">To Be Continued...</h1>
                    <p className="opacity-90">Happy Valentine's Day, Fred!</p>
                </div>
            </div>
        );
      default:
